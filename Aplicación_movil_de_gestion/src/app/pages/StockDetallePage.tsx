...
const CATEGORIAS_MOCK = ["Audio", "Cámaras", "Iluminación", "Trípodes", "Accesorios", "Otros"];

const ESTADO_META: Record<EstadoAlquilerStock, { label: string; bg: string; text: string; icon: React.ElementType }> = {
  activo:     { label: "Activo",     bg: "bg-green-100", text: "text-green-900", icon: Clock },
  vencido:    { label: "Vencido",    bg: "bg-red-100",   text: "text-red-900",   icon: AlertCircle },
  finalizado: { label: "Finalizado", bg: "bg-gray-100",  text: "text-gray-900",  icon: CheckCircle2 },
};
...
export default function StockDetallePage() {
  ...
  const [form, setForm] = useState({ nombre: "", categoria: "", disponibles: 0 });
  ...
  useEffect(() => {
    ...
    if (!found) { navigate("/app/stock", { replace: true }); return; }
    setItem(found);
    setForm({ 
      nombre: found.nombre, 
  ...
  const alquileresBase: AlquilerDeEquipo[] = alquileresPorItem[item.id] ?? [];
  ...
  const alquileresOrdenados = alquileresFiltrados.slice().sort((a, b) => {
  ...
  const cantidadAlquilados = Math.max(0, item.total - item.disponibles);
  ...
  const estadoStyle =
    estado === "disponible" ? "bg-green-100 text-green-900 border-green-400" :
    estado === "bajo"       ? "bg-amber-100 text-amber-950 border-amber-400" :
                              "bg-red-100 text-red-900 border-red-400";
  const estadoLabel =
    estado === "disponible" ? "Disponible" :
    estado === "bajo"       ? "Stock bajo" : "Agotado";
    ...
    <div className="pb-10 bg-gray-50 min-h-screen">
      ...
      <div className="bg-gradient-to-br from-[#1b6f5c] via-[#165a4b] to-[#0f3d33] px-5 pt-4 pb-6 text-white shadow-md">
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-white/20 border-2 border-white/40 flex items-center justify-center flex-shrink-0" aria-hidden="true">
            <Package size={26} className="text-white" />
          </div>
          <div className="min-w-0">
            <h1 className="text-xl font-extrabold leading-tight truncate">{item.nombre}</h1>
            <p className="text-white/80 text-sm mt-0.5 font-medium">
              Categoría: {item.categoria} · {alquileresBase.length} alquiler{alquileresBase.length !== 1 ? "es" : ""} en total
            </p>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSave} className="bg-white border border-gray-200 rounded-2xl p-5 space-y-4 shadow-sm">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, disponibles: Math.max(0, parseInt(e.target.value) || 0) }))}
              ...
              <p id={ayudaDisponiblesId} className="text-xs text-gray-500 mt-2 font-medium">
                * Hay <span className="font-bold text-gray-700">{cantidadAlquilados}</span> unidad{cantidadAlquilados !== 1 ? "es" : ""} retenida{cantidadAlquilados !== 1 ? "as" : ""} en alquileres activos. Modificar este valor recalculará el stock total.
              </p>
            </div>
            ...
            <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 mt-2">
              <div className="flex items-center justify-between mb-1.5">
                <span id={barraProgresoId} className="text-xs font-bold text-gray-600">Proporción de Stock Real</span>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                <div
                  ...
                  style={{ width: `${nuevoTotalCalculado > 0 ? (form.disponibles / nuevoTotalCalculado) * 100 : 0}%` }}
                  role="progressbar"
            ...
            >
              ...
              <span>{saved ? "¡Cambios guardados con éxito!" : "Guardar cambios"}</span>
            </button>
            <div className="sr-only" role="status" aria-live="polite">
              {saved && "El equipo ha sido actualizado correctamente en el sistema."}
            </div>
          </form>
        </section>
        ...
        <section aria-labelledby="alquileres-titulo" className="w-full space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
              ...
              <div className="relative flex items-center bg-white border-2 border-gray-300 rounded-xl h-14 shadow-sm flex-1 sm:w-56 focus-within:ring-4 focus-within:ring-[#218a72]/20 focus-within:border-[#165a4b] transition-all">
                ...
                <label htmlFor="filtro-estado" className="sr-only">Filtrar historial por estado de alquiler</label>
                <select
                  id="filtro-estado"
                  value={filtroEstado}
          ...
          >
            {alquileresPaginados.length === 0 ? (
              ...
              <div className="space-y-2.5" role="list" aria-label={`Lista de alquileres filtrados. Viendo página ${pagina} de ${totalPaginas}`}>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex-1 min-w-0" aria-hidden="true">
                          <p className="font-bold text-gray-990 text-sm truncate">{alq.cliente}</p>
