...
export default function StockDetallePage() {
        ...
        async function cargarDatos() {
            try {
                ...
                if (!resArticulo.ok) {
                    ...
                    try {
                        ...
                        switch (resArticulo.status) {
                            ...
                            case 500:
                                mensajeError =
                                    'Ocurrió un error interno en el servidor o en la base de datos SQLite.';
                                break;
                            default:
                ...
                const itemMapeado: StockItem = {
                    ...
                    disponibles:
                        articuloData.stock_total -
                        (articuloData.cantidad_alquilados || 0),
                    total: articuloData.stock_total,
                    precio_por_dia: Number(articuloData.precio_por_dia || 0),
                    deposito_garantia: Number(
                        articuloData.deposito_garantia || 0,
                    ),
                };
                ...
                const resAlquileres = await fetch(
                    `${API_BASE_URL}/alquileres/articulo/${id}`,
                    { headers: getAuthHeaders() },
                );
                if (resAlquileres.ok) {
    ...
    const cantidadAlquilados = item
        ? Math.max(0, item.total - item.disponibles)
        : 0;
    ...
    async function handleSave(e: React.FormEvent) {
        ...
        // Cambiar el return silencioso por una alerta explícita
        if (!form.nombre.trim()) {
            alert('El nombre del equipo no puede estar vacío.');
            return;
        }
        if (!item) return; // Esto solo pasa si no cargó el componente, está bien
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/stock/${item.id}`, {
                ...
                body: JSON.stringify({
                    ...
                    precio_por_dia: form.precio_por_dia,
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <p className="text-gray-500 font-bold animate-pulse text-sm">
                    Cargando especificaciones del equipo...
                </p>
            </div>
        );
    }
    ...
    const totalPaginas =
        Math.ceil(alquileresOrdenados.length / ITEMS_POR_PAGINA) || 1;
                    ...
                    <section aria-labelledby="editar-titulo" className="w-full">
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label
                                        htmlFor="edit-deposito-garantia"
                                        className="flex items-center gap-2 text-sm font-bold text-gray-800 mb-2"
                                    >
                                        <Tag
                                            size={15}
                            ...
                            <div>
                                ...
                                >
                                    * Hay{' '}
                                    ...
                                    unidad
                                    {cantidadAlquilados !== 1 ? 'es' : ''}{' '}
                                    retenida{' '}
                                    {cantidadAlquilados !== 1 ? 's' : ''} en
                                    alquileres activos. Modificar este valor
                        ...
                        >
                            {alquileresPaginados.length === 0 ? (
                                <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center text-gray-500 text-sm font-semibold shadow-sm">
                                    No hay alquileres registrados que coincidan
