...
function mapEstadoDetalle(
    estado: AlquilerApi['estado'],
    fechaFin: string,
): EstadoDetalle {
...
function InfoRow({
    ...
    return (
        <div className="flex items-start gap-4 py-3 border-b border-gray-100 last:border-0">
            ...
            <div
                className="w-8 h-8 rounded-lg bg-[#218a72]/10 flex items-center justify-center flex-shrink-0 mt-0.5"
                aria-hidden="true"
            >
            ...
            <div className="flex-1 min-w-0">
                ...
                <p
                    className={`text-sm font-semibold ${highlight ? 'text-[#165a4b]' : 'text-gray-900'}`}
                >
                    {value}
                </p>
            </div>
...
export default function AlquilerDetallePage() {
    ...
    const estadoVista = alquiler
        ? mapEstadoDetalle(alquiler.estado, alquiler.fecha_fin)
        : null;
        ...
        async function loadAlquiler() {
            ...
            try {
                const response = await fetch(
                    `${API_BASE_URL}/alquileres/${id}`,
                );
                if (!response.ok) {
                    throw new Error(
                        'No se pudo cargar el detalle del alquiler',
                    );
                }
        ...
        return (
            ...
            >
                <p className="text-gray-500 text-lg font-medium mb-4">
                    No se encontró el identificador del alquiler.
                </p>
                <button
    ...
    const estadoVistaSeguro = estadoVista ?? 'activo';

    const meta = ESTADO_META[estadoVistaSeguro];
    const EstadoIcon = meta.icon;
    const dias = calcularDias(alquiler.fecha_inicio, alquiler.fecha_fin);

    // Cálculos económicos
    const subtotal = itemsAlquilados.reduce(
        (acc: number, item: any) => acc + item.precio * item.cantidad * dias,
        0,
    );
    const totalOriginal = subtotal + Number(alquiler.deposito_garantia || 0);
    const cargosAdicionales =
        (tieneDanos ? cobroDanos : 0) + (fueraTermino ? cobroPenalizacion : 0);
    const balanceFinal = totalOriginal + cargosAdicionales;
    const totalPendiente = itemsAlquilados.reduce((acc, item) => {
        const devuelta = entregaParcial
            ? Number(cantidadesDevueltas[item.id] ?? item.cantidad)
            : item.cantidad;
    ...
    const handleProcesarEntrega = async (e: React.FormEvent) => {
        ...
        if (entregaParcial && totalPendiente === 0) {
            alert(
                'Si la entrega es parcial, al menos un ítem debe quedar con cantidad pendiente.',
            );
            return;
        }
        ...
        try {
            ...
            const payload = {
                observaciones: [
                    notasEntrega,
                    entregaParcial ? detalleParcial : '',
                    tieneDanos ? `Daños: ${detalleDanos}` : '',
                ]
                    .filter(Boolean)
                    .join(' | '),
                recargo_roturas: tieneDanos ? Number(cobroDanos || 0) : 0,
                recargo_tarde: fueraTermino
                    ? Number(cobroPenalizacion || 0)
                    : 0,
                recargo_otros: 0,
            ...
            const response = await fetch(
                `${API_BASE_URL}/alquileres/${id}/cierre`,
                {
                    method: 'PUT',
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    body.error || 'No se pudo registrar la devolución',
                );
            }
            ...
            alert(
                err instanceof Error
                    ? err.message
                    : 'Error inesperado al cerrar el alquiler',
            );
        } finally {
            ...
            <main className="px-4 -mt-6 max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                >
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <span
                                className={`text-xs font-black uppercase tracking-wider ${meta.text}`}
                                id="estado-contrato-titulo"
                            >
                                Estado del Contrato
                            </span>
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    >
                        <div className="px-5 pt-4 pb-3 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                            <h2
                                id="inventario-titulo"
                                className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2"
                            >
                                <Package
                                    size={14}
                                    className="text-[#218a72]"
                                    aria-hidden="true"
                                />{' '}
                                Detalle del Inventario Alquilado
                            </h2>
                            <span
                                className="bg-gray-200 text-gray-700 text-[10px] font-bold px-2 py-0.5 rounded-full"
                                aria-label={`${itemsAlquilados.length} elementos en total`}
                            >
                                {itemsAlquilados.length}{' '}
                                {itemsAlquilados.length === 1
                                    ? 'Ítem'
                                    : 'Ítems'}
                            </span>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <table
                                ...
                                aria-label="Lista detallada de equipos alquilados y costos individuales"
                            >
                                <thead>
                                    ...
                                    {itemsAlquilados.map(
                                        (item: any, idx: number) => (
                                            ...
                                            >
                                                ...
                                                <td className="py-3.5 px-4 text-center font-bold text-gray-600 bg-gray-50/40">
                                                    {item.cantidad}
                                                </td>
                    ...
                    {estadoVista !== 'finalizado' && (
                        <section
                            aria-labelledby="form-devolucion-titulo"
                            className="bg-white rounded-2xl border-2 border-[#218a72]/30 shadow-md overflow-hidden"
                        >
                            <div className="bg-gradient-to-r from-[#218a72] to-[#165a4b] px-5 py-3 text-white flex items-center gap-2">
                                <ClipboardCheck size={18} aria-hidden="true" />
                                <h2
                                    id="form-devolucion-titulo"
                            ...
                            <form
                                onSubmit={handleProcesarEntrega}
                            ...
                            >
                                ...
                                <fieldset className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        >
                                            ...
                                            <span className="text-xs font-bold text-gray-900">
                                                Completa (Tiempo y Forma)
                                            </span>
                                            <span className="text-[11px] text-gray-500 mt-0.5">
                                                Se devuelve todo lo solicitado.
                                            </span>
                                        </label>
                                    ...
                                    {entregaParcial && (
                                        <div
                                            id="seccion-detalle-parcial"
                                            className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-xl space-y-2"
                                        >
                                            <div className="flex gap-2 text-amber-800">
                                                ...
                                                />
                                                <label
                                                    htmlFor="textarea-parcial"
                                                    className="text-xs font-medium"
                                                >
                                                    Especifica detalladamente
                                                    qué elementos quedan
                                ...
                                {entregaParcial && (
                                    <section
                                        className="space-y-2"
                                        aria-label="Cantidades devueltas por ítem"
                                    >
                                ...
                                <div className="space-y-3">
                                    ...
                                    {tieneDanos && (
                                        <div
                                            id="seccion-detalle-danos"
                                            className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3 bg-rose-50 border border-rose-100 rounded-xl"
                                        >
                                            <div className="sm:col-span-1">
                                                <label
                                                    htmlFor="monto-danos"
                                                    className="text-[10px] font-bold text-rose-900 uppercase block mb-1"
                                                >
                                                    Monto de Recargo ($)
                                                </label>
                                                <input
                                ...
                                <div className="space-y-3">
                                    ...
                                    >
                                        ...
                                        <div className="text-xs">
                                            ...
                                            <span className="text-gray-400 text-[11px]">
                                                Aplicar recargos punitorios por
                                                entrega tardía
                                            </span>
                                        </div>
                                    </label>
                                    ...
                                    {fueraTermino && (
                                        <div
                                            id="seccion-detalle-retraso"
                                            className="p-3 bg-orange-50 border border-orange-100 rounded-xl flex flex-col sm:flex-row gap-3 items-end"
                                        >
                                            <div className="flex-1 w-full">
                                                <label
                                                    htmlFor="monto-penalizacion"
                                                    className="text-[10px] font-bold text-orange-900 uppercase block mb-1"
                                                >
                                                    Penalización Económica Total
                                                    ($)
                                                </label>
                                                <input
                                ...
                                <div className="space-y-1">
                                    <label
                                        htmlFor="notas-recepcion"
                                        className="text-[11px] font-bold text-gray-500 uppercase tracking-wider block"
                                    >
                                    ...
                                    <textarea
                                        ...
                                        value={notasEntrega}
                                        onChange={(e) =>
                                            setNotasEntrega(e.target.value)
                                        }
                ...
                <div className="space-y-6">
                    ...
                    >
                        <h2
                            ...
                            className="text-xs font-bold text-gray-500 uppercase tracking-wider px-5 pt-4 pb-1"
                        >
                        ...
                        <div className="px-5 pb-3">
                            <InfoRow
                                ...
                                value={formatFecha(alquiler.fecha_inicio)}
                            />
                            <InfoRow
                                icon={CalendarDays}
                                label="Pactado para devolver"
                                value={formatFecha(alquiler.fecha_fin)}
                            />
                            <InfoRow
                                icon={Clock}
                                label="Tiempo total"
                    ...
                    <section
                        aria-labelledby="liquidacion-titulo"
                        className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden"
                    >
                        <h2
                            id="liquidacion-titulo"
                            className="text-xs font-bold text-gray-500 uppercase tracking-wider px-5 pt-4 pb-1"
                        >
                        ...
                        <div className="px-5 pb-2">
                            <InfoRow
                                icon={Banknote}
                                label="Subtotal Neto Ítems"
                                value={formatMonto(subtotal)}
                            />
                            <InfoRow
                                icon={ShieldCheck}
                                label="Garantía de Resguardo"
                                value={
                                    Number(alquiler.deposito_garantia || 0) > 0
                                        ? formatMonto(
                                              Number(
                            ...
                            {cargosAdicionales > 0 && (
                                <div className="bg-rose-50/70 p-2.5 rounded-lg border border-rose-100 my-2 space-y-1 text-xs">
                                    <span className="font-bold text-rose-900 block text-[10px] uppercase">
                                        Recargos aplicados en la entrega:
                                    </span>
                                    {tieneDanos && (
                                        <div className="flex justify-between text-rose-800">
                                            <span>• Por Daños:</span>{' '}
                                            <span className="font-semibold">
                                                {formatMonto(cobroDanos)}
                                            </span>
                                        </div>
                                    )}
                                    {fueraTermino && (
                                        <div className="flex justify-between text-rose-800">
                                            <span>• Por Retraso:</span>{' '}
                                            <span className="font-semibold">
                                                {formatMonto(cobroPenalizacion)}
                                            </span>
                                        </div>
                                    )}
                                </div>
                        ...
                        >
                            <div className="flex flex-col">
                                ...
                                <span
                                    className="text-xs text-[#218a72] font-semibold"
                                    id="iva-informacion"
                                >
                                    Precios con IVA inc.
                                </span>
                            ...
                            <span
                                ...
                                aria-describedby="iva-informacion"
                            >
                    ...
                    {alquiler.notas && (
                        <section
                            aria-labelledby="notas-iniciales-titulo"
                            className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden"
                        >
                            <h2
                                id="notas-iniciales-titulo"
                                className="text-xs font-bold text-gray-500 uppercase tracking-wider px-5 pt-4 pb-1"
                            >
