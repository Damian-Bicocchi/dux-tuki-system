...
function initializeTables() {
    ...
    db.serialize(() => {
        ...
        // Migración idempotente para alquileres (deposito_garantia)
        db.all(`PRAGMA table_info(alquileres)`, (err, columns = []) => {
            if (err) return console.error('Error al leer columnas de alquileres:', err.message);
            const hasDeposito = columns.some((col) => col.name === 'deposito_garantia');
            if (!hasDeposito) {
                db.run(`ALTER TABLE alquileres ADD COLUMN deposito_garantia REAL NOT NULL DEFAULT 0`);
            }
        });
        ...
        // Migración idempotente para usuarios (is_admin y role_id)
        db.all(`PRAGMA table_info(usuarios)`, (err, columns = []) => {
            if (err) return console.error('Error al leer columnas de usuarios:', err.message);
            
            const hasIsAdmin = columns.some((col) => col.name === 'is_admin');
            const hasRoleId = columns.some((col) => col.name === 'role_id');

            if (!hasIsAdmin) {
                db.run(`ALTER TABLE usuarios ADD COLUMN is_admin INTEGER NOT NULL DEFAULT 0`);
            }
            if (!hasRoleId) {
                db.run(`ALTER TABLE usuarios ADD COLUMN role_id INTEGER REFERENCES roles(id)`);
            }
        });
        ...
        console.log('✅ Tablas e inicialización de BD completadas.');
    });
}
