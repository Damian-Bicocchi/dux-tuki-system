...
// 1. IMPORTAR MIDDLEWARES DE AUTENTICACIÓN Y PERMISOS
...
function estadoEfectivoAlquiler(alquiler) {
    if (!alquiler) return alquiler;

    const hoy = new Date().toISOString().split('T')[0];
    if (['devuelto', 'cancelado'].includes(alquiler.estado)) {
        return alquiler.estado;
    }

    if (alquiler.fecha_fin && alquiler.fecha_fin < hoy) {
        return 'vencido';
    }
...
function getAlquilerCompleto(id, callback) {
    db.get(
        ...
        (err, alquiler) => {
            ...
            alquiler.estado = estadoEfectivoAlquiler(alquiler);
...
// ── RUTAS DEFINIDAS CON SUS RESPECTIVOS PERMISOS ─────────────────────────────

// Lectura de alquileres vinculados a un artículo
router.get(
    '/articulo/:id',
    ...
    async (req, res) => {
        ...
        if (!articulo_id || !fecha_inicio || !fecha_fin) {
            return res.status(400).json({
                error: 'Faltan parámetros obligatorios (articulo_id, fecha_inicio, fecha_fin)',
            });
        }

        try {
            const articulo = await getAsync(
                'SELECT id, nombre, stock_total FROM articulos WHERE id = ?',
                [articulo_id],
            );
            if (!articulo) {
...
// Reporte de alquileres que empiezan
router.get(
    '/empiezan',
    checkPermission('permiso_para_ver_alquileres'),
    alquilerController.getAlquileresEmpiezanFecha
);

// Reporte de alquileres que terminan
router.get(
    '/terminan',
    checkPermission('permiso_para_ver_alquileres'),
    alquilerController.getAlquileresTerminanFecha
);
    ...
    (req, res) => {
        ...
        db.all(query, params, (err, rows) => {
            ...
            res.json(
                (rows || []).map((row) => ({
                    ...row,
                    estado: estadoEfectivoAlquiler(row),
                })),
            );
        });
    }
...
// Obtener un alquiler completo por ID
router.get(
    '/:id',
    checkPermission('permiso_para_ver_alquileres'),
    (req, res) => {
...
router.post(
    '/',
    checkPermission('permiso_para_registrar_alquileres'),
    async (req, res) => {
        const {
            cliente_id,
            fecha_inicio,
                            ...
                            db.get(
                                ...
                                (err, a) => {
                                    ...
                                    resolve({
                                        ...item,
                                        precio_unitario_dia:
                                            item.precio_unitario_dia !== undefined
...
// Editar alquiler completo
router.put(
    '/:id',
    checkPermission('permiso_para_editar_alquileres'),
    async (req, res) => {
        const {
            cliente_id,
            fecha_inicio,
...
// Guardar cierre del alquiler
router.put(
    '/:id/cierre',
    checkPermission('permiso_para_cerrar_alquileres'),
    async (req, res) => {
        ...
        try {
            ...
            const estadoAlquiler =
                estadoEntrega === 'cerrado' ? 'devuelto' : 'activo';
            ...
            try {
                ...
                await runAsync('UPDATE alquileres SET estado = ? WHERE id = ?', [
                    estadoAlquiler,
                    alquilerId,
                ]);
...
router.delete(
    '/:id',
    checkPermission('permiso_para_eliminar_alquileres'),
    (req, res) => {
        db.get(
