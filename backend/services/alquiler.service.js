...
class AlquilerService {
    ...
    async _verificarDisponibilidad(
        articulo_id,
        cantidad,
        fecha_inicio,
        fecha_fin,
        ...
        const articulo =
            await alquilerRepository.getStockTotalArticulo(articulo_id);
        if (!articulo) {
            throw new Error(
                `El equipo seleccionado ya no existe en el catálogo`,
            );
        }

        const ocupadas = await alquilerRepository.getCantidadOcupada(
            articulo_id,
            fecha_inicio,
            fecha_fin,
            excluir_alquiler_id,
        ...
        if (disponibles < cantidad) {
            if (disponibles <= 0) {
                throw new Error(
                    `No hay stock disponible del equipo "${articulo.nombre}" para las fechas seleccionadas.`,
                );
            }
            throw new Error(
    ...
    async listarAlquileres(filtros) {
        const rows =
            await alquilerRepository.listarAlquileresFiltrados(filtros);
        return rows.map((row) => ({
            ...row,
            estado: this._estadoEfectivo(row),
        }));
    }
    ...
    async crearAlquiler(data) {
        ...
        for (const item of items) {
            await this._verificarDisponibilidad(
                item.articulo_id,
                item.cantidad,
                fecha_inicio,
                fecha_fin,
        ...
        const itemsConPrecio = await Promise.all(
            items.map(async (item) => {
                const art = await alquilerRepository.getStockTotalArticulo(
                    item.articulo_id,
                );
                return {
                    ...item,
        ...
        await alquilerRepository.insertarItemsAlquiler(
            alquiler_id,
            itemsConPrecio,
        );
    ...
    async actualizarAlquilerCompleto(alquiler_id, data) {
        ...
        // 3. Ejecutar Update
        const cambios = await alquilerRepository.actualizarAlquiler(
            alquiler_id,
            {
                cliente_id,
                fecha_inicio,
    ...
    async cambiarEstado(id, estado) {
        const cambios = await alquilerRepository.actualizarEstadoAlquiler(
            id,
            estado,
        );
        if (cambios === 0) return null;
    ...
    async listarAlquileresPorArticulo(articuloId) {
        const rows =
            await alquilerRepository.findAlquileresByArticuloId(articuloId);

        return rows.map((row) => {
            const estadoBackend = this._estadoEfectivo(row);
            let estadoFrontend = 'finalizado';
            if (estadoBackend === 'vencido') {
                estadoFrontend = 'vencido';
            } else if (
                ...
                estadoBackend === 'devuelto' ||
                estadoBackend === 'cancelado'
            ) {
